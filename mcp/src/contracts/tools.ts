...
const array = (items: FieldSchema, maxItems = 100): FieldSchema => ({ type: "array", items, minItems: 0, maxItems });
...
export const TOOL_CONTRACTS: readonly McpToolContract[] = [
  ...
  readTool({ name: "get_schema", group: "discovery", summary: "Obtém schema público de uma ferramenta.", scopes: [], input: object({ tool_name: f.text }, ["tool_name"]), data: single("schema", object({ tool_name: f.text, input_schema: object({ type: f.text }, ["type"]) }, ["tool_name", "input_schema"])), example: { tool_name: "listar_lancamentos" }, output: { schema: { tool_name: "listar_lancamentos", input_schema: { type: "object" } } } }),
  ...
  ...["atualizar_recorrencia", "pausar_recorrencia", "retomar_recorrencia", "cancelar_recorrencia"].map((name) => writeTool({ name, group: "recorrencias", summary: "Altera estado ou regra da recorrência.", scopes: [name === "cancelar_recorrencia" ? "finance:delete" : "finance:update"], input: object({ id: f.uuid, patch: patchRecorrencia, motivo: f.note }, ["id"]), data: single("recorrencia", recorrencia), example: { id: uuid, patch: { frequencia: "mensal" }, expected_version: 1, confirmation_token: token, dry_run: false }, output: { recorrencia: { id: uuid, version: 2, frequencia: "mensal", inicio: "2026-09-01", status: name === "pausar_recorrencia" ? "pausada" : name === "cancelar_recorrencia" ? "cancelada" : "ativa" } }, expectedVersion: true })),
  ...
  ...["atualizar_banco", "arquivar_banco", "reativar_banco", "atualizar_categoria", "mover_categoria", "arquivar_categoria", "reativar_categoria", "atualizar_cliente_credor", "arquivar_cliente_credor", "mesclar_clientes_credores"].map((name) => writeTool({ name, group: "cadastros", summary: "Altera cadastro preservando versão e histórico.", scopes: ["admin:cadastros"], input: object({ id: f.uuid, destino_id: f.uuid, categoria_pai_id: f.uuid, patch: patchCadastro }, ["id"]), data: single("cadastro", cadastro), example: { id: uuid, patch: { nome: "Banco Principal" }, expected_version: 1, confirmation_token: token, dry_run: false }, output: { cadastro: { id: uuid, version: 2, nome: "Banco Principal", arquivado: name.includes("arquivar") } }, expectedVersion: true })),

  readTool({ name: "simular_lote", group: "lotes", summary: "Simula lote sem persistir efeitos.", scopes: ["finance:update"], input: object({ operacoes: array(object({ tool: f.text, payload: openObject() }, ["tool", "payload"]), 250) }, ["operacoes"]), data: single("simulacao", object({ validas: { type: "integer", minimum: 0 }, invalidas: { type: "integer", minimum: 0 } }, ["validas", "invalidas"])), example: { operacoes: [{ tool: "atualizar_lancamento", payload: { id: uuid } }] }, output: { simulacao: { validas: 1, invalidas: 0 } } }),
  writeTool({ name: "executar_lote", group: "lotes", summary: "Executa lote confirmado e idempotente.", scopes: ["finance:update"], input: object({ operacoes: array(object({ tool: f.text, payload: openObject() }, ["tool", "payload"]), 250) }, ["operacoes"]), data: single("lote", object({ id: f.uuid, status: { type: "string", enum: ["concluido", "parcial"] } }, ["id", "status"])), example: { operacoes: [{ tool: "atualizar_lancamento", payload: { id: uuid } }], confirmation_token: token, dry_run: false }, output: { lote: { id: uuid, status: "concluido" } }, maxBatchItems: 250 }),
...
export const CONNECTED_MCP_HANDLERS = {} as const satisfies Readonly<Record<string, McpToolHandler>>;
