...
- **Janela de tipagem vermelha, esperada, da Task 3 até a Task 12.** Ao tornar `tenant_id` NOT NULL sem default, o tipo `Insert` gerado passa a exigir o campo, e todo hook que insere sem ele deixa de compilar. São 8 erros `TS2769` em `useApiKeys`, `useBancos`, `useCategorias`, `useChatMessages`, `useLancamentos`, `useTransferencia` (2x) e `useUserPermissions`. O trigger da Task 5 preenche o valor no banco, mas não altera o tipo gerado — só a Task 12, ao fazer os hooks enviarem `tenant_id` explicitamente, fecha isso. Nesse intervalo, `npx tsc -p tsconfig.app.json --noEmit` falha e `npm run build` também: é o comportamento correto, não regressão. O que se exige de cada task nessa janela é que o conjunto de erros **não cresça** além desses 8. A partir da Task 12, a tipagem volta a ter de passar limpa.
