...
-- A identity nasce com start_value = 1 e ignora os dados existentes. Sem este
-- setval, em produção — onde a linha id = 1 já existe — o próximo insert colide
-- na PK. Funciona nos dois estados: tabela vazia resulta em próximo valor 1.
SELECT setval(pg_get_serial_sequence('public.ai_settings','id'),
              COALESCE((SELECT max(id) FROM public.ai_settings), 0) + 1, false);
